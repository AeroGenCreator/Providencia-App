...
def margen_beneficio_bruto_hoy():
    try:
        ...
        if venta_total > 0:
            margen = round((beneficio_bruto/venta_total) * 100,2)
            return margen
        else:
            return 0
    except KeyError:
...
with tab_1:
    ...
    col_1.image('pro.jpg',width=100,caption=l.phrase[7])
    ...
    if corte:
        class PDF(FPDF):
            def __init__(self, orientation = "portrait", unit = "mm", format = "letter"):
                ...
                self.add_font(family='ArialUnicodeMS',fname='arial-unicode-ms.ttf')
            def header(self):
                self.set_text_color(0,0,0)
                self.set_font(family='ArialUnicodeMS',size=20)
                self.image('pro.jpg', 10, 8, 16)
                self.cell(20,6,text=l.phrase[14], border=False,center=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
                self.ln(15)
            def footer(self):
                self.set_y(-10)
        ...
        pdf.cell(65,6,text=f'Fecha: {HOY}',border=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        pdf.cell(0,4,fill=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)

        # Creo los nombre de las columnas, las cuales iran por encima de los datos de tabla
        pdf.cell(65,6,text=l.phrase[13],border=True,align='C')
        pdf.cell(65,6,text=l.phrase[12],border=True,align='C')
        pdf.cell(66,6,text=l.phrase[8],border=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        
        # Creo una linea muerta como separador
        pdf.cell(0,2,fill=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)

        pdf.cell(65,6,text=f'{total_en_caja}',border=True,align='C')
        pdf.cell(65,6,text=f'{fondo}',border=True,align='C')
        pdf.cell(66,6,text=f'{venta}',border=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        ...
        pdf.cell(0,4,fill=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        pdf.cell(half_page,6,text=l.phrase[10], border=True, align='C')
        pdf.cell(half_page,6,text=l.phrase[11], border=True, align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)

        pdf.cell(half_page,6,text=f'{beneficio}',border=True, align='C')
        pdf.cell(half_page,6,text=f'{margen_beneficio}',border=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
