...
def generar_factura_pdf(folio_name, folio_number, discount_percentage):
    ...
    class PDF(FPDF):
        ...
        def header(self):
            ...
            self.cell(0,6,text=l.phrase[61], border=False,align='C', new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        def footer(self):
            self.set_y(-10)
    ...
    pdf.cell(0,6,text=f'{l.phrase[68]}: {folio_name}',align='C',border=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    pdf.cell(half_page,6,text=f'{l.phrase[22]}: {folio_number}',align='C',border=True)

    pdf.set_text_color(64,64,64)
    pdf.cell(half_page,6,text=f'{l.phrase[30]}: {HOY.isoformat()}',align='C',border=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    pdf.cell(0,1,fill=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    
    pdf.cell(12,6,text='PIEZAS',border=True,align='C')
    pdf.cell(16,6,text='MEDIDA',border=True,align='C')
    pdf.cell(83.5,6,text='PRODUCTO',border=True,align='C')
    pdf.cell(17,6,text='TOTAL',border=True,align='R',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    ...
    for index, row in st.session_state.df_c.iterrows():
        pdf.cell(12,6,text=f'{row['Unidades']}',border=True,align='C')
        pdf.cell(16,6,text=f'{row['Medida']}',border=True,align='C')
        pdf.cell(83.5,6,text=f'{row['Producto']}',border=True)
        pdf.cell(17,6,text=f'$ {round(row['Total'],2)}',border=True,align='R',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    
    pdf.set_font(family='ArialUnicodeMS',size=8.5)
    # Determino formatos e imprimo los contadores al final de la tabla.
    pdf.cell(0,1,fill=True,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
    pdf.cell(half_page,6,text=f'{l.phrase[23]}: {st.session_state.productos}', border=True, align='C')
    pdf.cell(half_page,6,text=f'{l.phrase[24]}: $ {round(st.session_state.total,2)}', border=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)

    pdf.cell(half_page,6,text=f'{l.phrase[63]}: %{round(discount_percentage * 100)}',border=True,align='C')
    pdf.set_text_color(0,0,255)
    pdf.cell(half_page,6,text=f'{l.phrase[62]}: $ {round(st.session_state.total_desc,2)}',border=True,align='C',new_x=XPos.LMARGIN,new_y=YPos.NEXT)
...
def por_facturar():
    ...
    try:
        ...
        RUTA_SEGURIDAD = 'copias_seguridad.json'
        ...
        # 1. Asegúrate de que el archivo no esté vacío antes de intentar cargarlo.
        if os.path.exists(RUTA_PRODUCTOS) and os.path.getsize(RUTA_PRODUCTOS) > 0:
            with open(RUTA_PRODUCTOS, 'r', encoding='utf-8') as file:
                # La línea que más probablemente causa el error si el archivo está corrupto/vacío
                df = pd.DataFrame(data=json.load(file))
        else:
            # Si el archivo está vacío o no existe, manejalo sin lanzar excepción.
            st.error("⚠️ Error: El archivo de inventario está vacío o no se encuentra.")
            return # Detiene la función si el inventario no se puede cargar
        ...
        # Asigno contadores al cache de streamlit
        ...
        # Botones para copia de seguridad
        col4, col5 = st.columns(2)
        with col4:
            copia_de_seguridad = st.button(
                label=l.phrase[104],
                key='copia_de_seguridad',
                type='secondary',
                width='stretch',
            )
        with col5:
            cargar_copia_seguridad = st.button(
                label=l.phrase[105],
                key='cargar_copia_seguridad',
                type='primary',
                width='stretch'
            )
        ...
        if cargar_copia_seguridad:
            try:
                if os.path.exists(RUTA_SEGURIDAD) and os.path.getsize(RUTA_SEGURIDAD) > 0:
                    with open(RUTA_SEGURIDAD,'r',encoding='utf-8') as f:
                        datos = json.load(f)
                        diccionario = datos['factura']
                        df_respaldo = pd.DataFrame(diccionario)
                        st.session_state.df_c = df_respaldo
                else:
                    return
            except KeyError:
                st.warning(l.phrase[107])
        ...
        if calcular_total or imprimir or send_email or copia_de_seguridad:
        ...
        if imprimir:
            ...
            with open('facturas_historial.json','r',encoding='utf-8') as f:
                facturas_historial = json.load(f)
                facturas_historial[HOY.isoformat()+'_'+str(nombre_factura)+'_'+str(FOLIO_FACTURA)+'_'+str(DESCUENTO_PORCENTAJE)] = st.session_state.df_c.to_dict(orient='list')
            with open('facturas_historial.json','w',encoding='utf-8') as e:
                json.dump(facturas_historial,e,indent=2,ensure_ascii=False)

            return
        
        # Aqui agregamos respaldo para la factura en proceso:
        if copia_de_seguridad:
            df_seguridad = st.session_state.df_c.copy()
            diccionario_seguridad =  df_seguridad.to_dict(orient='list')
            try:
                if os.path.exists(RUTA_SEGURIDAD) and os.path.getsize(RUTA_SEGURIDAD) > 0:
                    with open(RUTA_SEGURIDAD,'r',encoding='utf-8') as f:
                        datos = json.load(f)
                        datos['factura'] = diccionario_seguridad
                    with open(RUTA_SEGURIDAD,'w',encoding='utf-8')as e:
                        json.dump(datos,e,indent=2,ensure_ascii=False)
                        st.success(l.phrase[107])
                        return
                else:
                    creacion = {'factura':diccionario_seguridad}
                    with open(RUTA_SEGURIDAD,'w',encoding='utf-8') as c:
                        json.dump(creacion,c,indent=2,ensure_ascii=False)
                        st.success(l.phrase[107])
                        return
            except(KeyError):
                st.warning(l.phrase[106])

        if send_email:
            pdf_bytes_email = generar_factura_pdf(nombre_factura,FOLIO_FACTURA,DESCUENTO_PORCENTAJE)

            subject = l.phrase[68]
            port = 465 # (465 SSL para gmail) (587 SSL para outlook)
            smtp_server = "smtp.gmail.com" #smtp.gmail.com / smtp-mail.outlook.com
            remitente = st.secrets['email']['user']
            password = st.secrets['email']['password']
            destino = st.secrets['email']['destinatarios']
            body = f'Ferreteria La Providencia: La siguiente informacion debe facturarse.' \
            'Fecha de creacion de factura: {HOY}. El PDF contiene los datos de facturacion y los productos por facturar. ' \
            'El descuento se aplica al total. Esta factura es oficial sin embargo confirmar antes de realizar.'
    ...
    #Si hay problemas con la lectura se advierte la probable falta de datos.
        ...
        # Captura específicamente el error de JSON corrupto
        st.error(f"❌ Error de formato en el archivo JSON: {RUTA_PRODUCTOS}. Revisa su contenido.")
    except Exception as e:
        # Captura otros errores inesperados
        st.warning(f"Advertencia general al acceder al inventario: {e}")

def historial():
    try:
        with open('facturas_historial.json','r',encoding='utf8') as f:
            datos = json.load(f)
            llaves = list(datos.keys())
    except FileNotFoundError:
        st.error('No existe el archivo')
    seleccion = st.selectbox(label='Facturas:',options=llaves)
    if seleccion:
        cargar = st.button(label='Cargar Factura Seleccionada',width='stretch',type='primary')
        if cargar:
            st.session_state.df_c = datos[seleccion]
            st.rerun()
#-------------------------------------------------------------------------------------------
...
st.title(f':material/request_quote: {l.phrase[61]}')
tab1, tab2 = st.tabs(['Facturacion','Historial de Facturas'])
with tab1:
    por_facturar()
with tab2:
    historial()